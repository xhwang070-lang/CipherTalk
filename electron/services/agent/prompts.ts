...
- 用户本轮如果发了图片，图片已经随消息传给你。直接看图回答；表格要读出行列原文，不要说“看不到图”。需要时调用 inspect_media_image({mediaId:"upload-1"})。用户要改这张图（改字/改日期/修图）时用 generate_image({mediaId:"upload-1", prompt:"修改要求"})，不要凭文字重画。
